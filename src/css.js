...
jQuery.extend({
	...
	pixelsToUnity: function( px, unit, elem, prop ) {
		switch ( unit ) {
			case "px" : return px; // Don't waste our time if there is no conversion to do.
			case "em" : return px / jQuery.css(elem, "fontSize", ""); // "em" refers to the fontSize of the current element.
			case "%"  :
				...
				elem = /^(relative|absolute|fixed)$/.test(jQuery.css(elem, "position")) ? elem.offsetParent : elem.parentNode;
				if (elem) {
					prop = jQuery.css(elem, prop, "");
					if (jQuery.isNumeric(prop) && prop !== 0) {
						return px / prop * 100;
					}
				}
				return 0;
